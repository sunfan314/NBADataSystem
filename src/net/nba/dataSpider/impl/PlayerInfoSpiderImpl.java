...
public class PlayerInfoSpiderImpl implements PlayerInfoSpider {
			...
			if (matcher.matches()) {
				...
				while (matcher.find()) {
					...
					if (matcher1.find()) {//获取球员身高
						...
						if(matcher2.matches()){
							player.setHeight(Double.parseDouble(matcher2.group(1)));
						}
					}
					if (matcher1.find()) {//获取球员体重
						...
						Matcher matcher2=pattern2.matcher(str3);
						if(matcher2.matches()){
							player.setWeight(Double.parseDouble(matcher2.group(1)));
						}
					}
					...
					if (matcher1.find()) {//获取球员球龄
						...
						Matcher matcher2=pattern2.matcher(str3);
						if(matcher2.matches()){
							player.setYearInNBA(Integer.parseInt(matcher2.group(1)));
						}
					}
					pattern1 = Pattern
	...
	public void downloadPlayerPic(List<String> nameInEnList) {
		...
		String nameStr=nameInEnList.get(0);
		String urlStr = DataSourceUrl.getPlayerDetailInfoURL(nameStr);
		StringBuffer webPageBuffer = WebPageReader.readWebPage(urlStr);
		pattern=Pattern.compile("(.*)(<!-- 个人信息 begin -->)(.*?)(<!-- 个人信息 end -->)(.*)");
		matcher=pattern.matcher(webPageBuffer);
		if(matcher.matches()){
			String str1=matcher.group(3);
